    ...
    with wandb.init(config=config) as run:
        ...
        for epoch in range(config.epochs):
            ...
            for train_index in range(len(train_data_given)):
                ...
                input_vector = word_to_tensor(eng_alphabet, train_data_given[train_index])
                ...
                for char in input_vector:
                    if config.cell_type == 'lstm':
                        output_enc, (hidden_enc, cell_enc) = encoder(char.to(device, non_blocking=True), (hidden_enc, cell_enc))
                    else:
                        output_enc, hidden_enc = encoder(char.to(device, non_blocking=True), hidden_enc)
                
                input_dec = torch.tensor([[0]])
                target_vector = word_to_tensor(tam_alphabet, train_data_target[train_index])
                ...
                if use_teacher_forcing:
                    for di in range(target_len):
                        if config.cell_type == 'lstm':
                            output_dec, (hidden_dec, cell_dec) = decoder(input_dec.to(device, non_blocking=True), (hidden_dec, cell_dec))
                        else:
                            output_dec, hidden_dec = decoder(input_dec.to(device, non_blocking=True), hidden_dec)
                        loss += loss_fn(output_dec, target_vector[di].to(device, non_blocking=True))
                        input_dec = target_vector[di]
                        prediction.append(output_dec.argmax(dim=1).squeeze().detach())
                else:
                    for di in range(target_len):
                        if config.cell_type == 'lstm':
                            output_dec, (hidden_dec, cell_dec) = decoder(input_dec.to(device, non_blocking=True), (hidden_dec, cell_dec))
            ...
            with torch.inference_mode(): 
                for val_index in range(len(val_data_given)):
                    input_vector = word_to_tensor(eng_alphabet, val_data_given[val_index])
                    input_len = len(input_vector)
                    hidden_enc = encoder.initHidden().to(device, non_blocking=True)
                    cell_enc = encoder.initHidden().to(device, non_blocking=True)
                    ...
                    input_dec = torch.tensor([[0]])
                    target_vector = word_to_tensor(tam_alphabet, val_data_target[val_index])
                    hidden_dec = torch.cat([hidden_enc[-1].reshape(1, 1, -1)]*decoder.num_layers).to(device, non_blocking=True)
                    if config.cell_type == 'lstm':
                        cell_dec = torch.cat([cell_enc[-1].reshape(1, 1, -1)]*decoder.num_layers).to(device, non_blocking=True)
                    ...
                    while max_length > 0:
                        ...
                        prediction.append(pred_char_index)
...
wandb.agent(sweep_id, perform_run, project='CS6910-A3', count=1)
